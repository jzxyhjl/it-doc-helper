...
2. 创建数据库：
```sql
CREATE DATABASE it_doc_helper;
CREATE USER it_doc_helper WITH PASSWORD 'your_password';
GRANT ALL PRIVILEGES ON DATABASE it_doc_helper TO it_doc_helper;
```
...
4. 配置 `.env` 文件：
```env
DATABASE_URL=postgresql://it_doc_helper:your_password@localhost:5432/it_doc_helper
```
...
数据存储在Docker Volume中：
- Volume名称：`it-doc-helper_postgres_data`
- 数据位置：Docker管理的卷中
...
```bash
# 进入PostgreSQL容器
docker exec -it it-doc-helper-postgres psql -U it_doc_helper -d it_doc_helper

# 或使用本地客户端连接
psql -h localhost -U it_doc_helper -d it_doc_helper
```
